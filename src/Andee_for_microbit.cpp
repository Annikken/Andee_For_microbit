...
BLEPeripheral andeePeripheral = BLEPeripheral(BLE_REQ, BLE_RDY, BLE_RST);

// create service
BLEService andeeService = BLEService("516e7d03-c4ea-4103-9bd2-c560221a0c16");
// create counter characteristic
BLECharacteristic AndeeTx = BLECharacteristic("516e7d04-c4ea-4103-9bd2-c560221a0c16", BLEWrite | BLEWriteWithoutResponse | BLENotify, 20 /*| BLEIndicate*/);
BLECharacteristic AndeeRx = BLECharacteristic("516e7d05-c4ea-4103-9bd2-c560221a0c16", BLEWrite | BLEWriteWithoutResponse | BLENotify, 20 /*| BLEIndicate*/);
...
void read_callback(BLECentral& central, BLECharacteristic& characteristic)
{
  unsigned int mLen;
  unsigned int rLen;
  char buffer[20];
...
void versionNumber(void)
{
	...
	versionBuff[3] = Andee_microbitVersion[0];
	versionBuff[4] = Andee_microbitVersion[1];
	versionBuff[5] = Andee_microbitVersion[2];
	versionBuff[6] = Andee_microbitVersion[3];
	versionBuff[7] = Andee_microbitVersion[4];
...
void AndeeClass::begin()
{
	...
	andeePeripheral.addAttribute(AndeeTx);
	andeePeripheral.addAttribute(AndeeRx);

	// assign event handlers for connected, disconnected to peripheral
	andeePeripheral.setEventHandler(BLEConnected, blePeripheralConnectHandler);
	andeePeripheral.setEventHandler(BLEDisconnected, blePeripheralDisconnectHandler);
	...
	Serial.println("Andee microbit Ready!");
}
